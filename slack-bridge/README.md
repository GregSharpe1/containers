...
Configure Slack event subscriptions for `app_mention` and the matching message event so the bridge receives unmentioned thread replies: `message.channels` for public channels or `message.groups` for private channels. The app needs the corresponding channel-history permission (`channels:history` or `groups:history`). On the first mention in an existing thread, the bridge loads the root message and all earlier replies as OpenCode context.
